...
// *** FÊRKIRÎ (Function Updated) ***
function showPage(pageId, pageTitle = '') {
    // --- DAXWAZA 2 (Beşa Tomarkirinê) ---
    // Berî guhertina rûpelê, pozîsyona scroll-ê tomar bike
    // Tenê heke em ji rûpela sereke (mainPage) derdikevin
    if (mainPage.classList.contains('page-active') && pageId !== 'mainPage') {
...
        btn.onclick = async () => {
            await handleCategorySelection(cat.id); // Vê yekê logîk û renderkirinê dike

            // --- DAXWAZA 1 ---
            // Piştî klîkkirinê, scroll bike da ku jêr-beşan nîşan bide
            const subcategoriesContainer = document.getElementById('subcategoriesContainer');
            if (subcategoriesContainer && subcategoriesContainer.style.display !== 'none') {
                // Scroll berbi destpêka konteynirê jêr-beşan
                window.scrollTo({
                    top: subcategoriesContainer.offsetTop - 80, // 80px buffer ji bo header
                    behavior: 'smooth'
                });
            } else if (state.currentCategory !== 'all') {
                 // Heke jêr-beş tune bin (ji bilî 'all'), tenê biçe jorê lîsteya berheman
                  const productsContainer = document.getElementById('productsContainer');
                  if(productsContainer) {
                      window.scrollTo({ top: productsContainer.offsetTop - 80, behavior: 'smooth' });
                  }
            }
            // Heke 'all' hat klîkkirin, ew ê jixwe here jor (ji ber updateProductViewUI)
        };
...
    // --- GUHERÎN Lİ VIR ---
    // 'preventScroll: true' lê zêde bike da ku em bikaribin scroll-ê
    // bi destan di 'renderMainCategoriesUI' de kontrol bikin
    const options = { 
        preventScroll: categoryId !== 'all' // Scroll-ê ranegire heke 'all' hat klîkkirin
    };
    await updateProductViewUI(true, options); // true indicates a new filter/search
}

// *** FÊRKIRÎ (Function Updated) ***
// Handles applying the current filter state to the UI (fetching & rendering)
async function updateProductViewUI(isNewSearch = false, options = {}) {
    // --- GUHERÎN Lİ VIR ---
    // Vebijarkên nû ji bo kontrolkirina scroll-ê
    const { restoreScrollPosition = null, preventScroll = false } = options;
    // --- Dawiya Guherînê ---
...
     // --- Logica Scroll-ê ya Nûvekirî (DAXWAZA 1 & 2) ---
    if (typeof restoreScrollPosition === 'number') {
        // DAXWAZA 2: Heke ji 'popstate' tê, pozîsyonê vegerîne
        setTimeout(() => window.scrollTo(0, restoreScrollPosition), 50);
    } else if (isNewSearch && !preventScroll) {
        // DAXWAZA 1 (Fallback): Heke lêgerînek nû ye Û me negotîye "scroll neke"
        window.scrollTo({ top: 0, behavior: 'smooth' });
    }
    // Heke preventScroll = true, an 'infinite scroll', tiştek neke.
}
...
    products.forEach(product => {
        const card = createProductCardElementUI(product);
  S      productsScroller.appendChild(card);
    });
...
S                loader.style.display = 'none'; // Hide loader after fetching
...
i       if(state.currentCategory === 'all' && !state.currentSearch) {
...
S   if (document.getElementById('favoritesSheet').classList.contains('show')) renderFavoritesPageUI();
...
                     if (subCatSnap.exists()) {
                         const subCat = subCatSnap.data();
SC                      pageTitle = subCat['name_' + state.currentLanguage] || subCat.name_ku_sorani || 'Details';
...
          G  showPage(popState.id, pageTitle);
...
            
            // --- DAXWAZA 2 (Beşa Cîbicîkirinê) ---
            // Em 'true' ji bo isNewSearch derbas dikin da ku lîsteyê ji nû ve bar bike,
            // lê em pozîsyona scroll-ê jî derbas dikin da ku ew neçe jor.
            const scrollPos = (typeof popState.scroll === 'number') ? popState.scroll : null;
            await updateProductViewUI(true, { restoreScrollPosition: scrollPos }); 
            // --- Dawiya Koda Nû ---
        }
    } else {
...
V        else { element.textContent = translation; }
    });
...
S   handleInitialPageLoadUI(); // Categories should be ready now
...
         } else {
iOS           // Fallback to main page if categories aren't ready (should be rare now)
...
Sunday         subcategory: params.get('subcategory') || 'all',
...
S           const isModal = element.classList.contains('modal');
             if (isSheet || isModal) {
...
Indicates   // For now, using direct fetch here, but ideally from core
     try {
...
        S     const linkElement = document.createElement('a');
...
s                       profileAddressInput.value = data.display_name;
                          showNotification('ناونیشان وەرگیرا', 'success');
                      } else {
                S         showNotification('نەتوانرا ناونیشان بدۆزرێتەوە', 'error');
OS               }
                  } catch (error) {
                      console.error('Reverse Geocoding Error:', error);
aS                   showNotification('هەڵەیەک لە وەرگرتنی ناونیشان ڕوویدا', 'error');
          S       } finally {
...
S                     case 3: message = 'کاتی داواکارییەکە تەواو بوو'; break;
                  }
                  showNotification(message, 'error');
                   if(btnSpan) btnSpan.textContent = originalBtnText;
          S       getLocationBtn.disabled = false;
              }
         );
     });
}
