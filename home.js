...
// === KODA ÇAKKIRÎ: Parametera scrollToPosition lê zêde bû ===
export async function updateProductViewUI(isNewSearch = false, scrollToPosition = null) {
...
        // For home view, show a loader *inside* the home container ONLY if cache MISSES
        // If cache hits, content will be rendered quickly without loader flash
        if (!getHomeCacheData('homeLayout')) { // Check if layout is cached
...
        } else {
            homeSectionsContainer.innerHTML = ''; // Clear potentially old content if cache hits
        }
...
        // Use '|| !homeSectionsContainer.hasChildNodes()' for safer empty check
        if (isNewSearch || !homeSectionsContainer.hasChildNodes() || homeSectionsContainer.querySelector('#loader')) {
...
    // === KODA ÇAKKIRÎ: Logika Scrollê ===
    // Piştî ku naverok hate nîşandan, scroll bike
    if (typeof scrollToPosition === 'number' && scrollToPosition >= 0) {
        // Use setTimeout to ensure the browser has time to render the content before scrolling
        setTimeout(() => window.scrollTo({ top: scrollToPosition, behavior: 'instant' }), 0); 
    } else if (isNewSearch && scrollToPosition === null) { 
        // Tenê dema ku lêgerînek/fîlterek *nû* tê kirin, scroll bike jor
...
    // Show loader inside the container if it's empty AND cache misses
    if (!homeSectionsContainer.hasChildNodes() && !getHomeCacheData('homeLayout')) {
        homeSectionsContainer.innerHTML = `<div id="loader" style="text-align: center; padding: 40px; color: var(--dark-gray); display: block;"><i class="fas fa-spinner fa-spin fa-2x"></i><p style="margin-top: 10px;">...خەریکی بارکردنی بەشەکانە</p></div>`;
    }

    const layout = await fetchHomeLayout(); // Fetch layout from core (will use cache if available)
