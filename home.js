...
// *** تکایە دڵنیابە ناوی فایلەکە ڕاستە (app-ui.js یان ui-render.js) ***
import {
    renderSkeletonLoader, createProductCardElementUI, setupScrollAnimations, showSubcategoryDetailPageUI
} from './ui-render.js'; // *** ئەگەر ناوی فایلەکەت ui-render.js بوو، بیکە بەم شێوەیە ***
// import {
//     renderSkeletonLoader, createProductCardElementUI, setupScrollAnimations, showSubcategoryDetailPageUI
// } from './app-ui.js'; // *** ئەگەر ناوی فایلەکەت app-ui.js بوو، ئەمە بەکاربێنە ***


// --- Cache Configuration ---
const HOME_CACHE_KEY = 'homePageContentCache';
...
        // For home view, check cache first (renderHomePageContentUI handles this)
        // Don't show skeleton here, let renderHomePageContentUI handle loader/cache
        productsContainer.style.display = 'none';
        skeletonLoader.style.display = 'none';
        scrollTrigger.style.display = 'none';
        homeSectionsContainer.style.display = 'block'; // Make sure container is visible
    }
...
        homeSectionsContainer.style.display = 'block'; // Ensure home sections container is visible
        // Render home content (will use cache if available and not expired)
...
// Function to render home page sections (UI Part) - Modified for Caching
export async function renderHomePageContentUI() {
    const homeSectionsContainer = document.getElementById('homePageSectionsContainer');
    if (!homeSectionsContainer) return;

    // --- Check Cache ---
    const cachedData = JSON.parse(localStorage.getItem(HOME_CACHE_KEY));
    const now = Date.now();

    if (cachedData && (now - cachedData.timestamp < CACHE_DURATION_MS)) {
        console.log("Rendering home page from cache.");
        // Render directly from cached HTML (Simple approach)
        homeSectionsContainer.innerHTML = cachedData.htmlContent;
        // Re-attach necessary event listeners or re-initialize sliders if needed
        // Re-initialize sliders (Example: Needs specific logic based on how sliders are built)
        // This is a simplified placeholder - real implementation might be more complex
        reinitializeSlidersFromCache(homeSectionsContainer);
        setupScrollAnimations(); // Still need scroll animations
        return; // Exit function, rendered from cache
    }
    console.log("Cache expired or not found. Fetching fresh home page data.");
...
        if(allProductsSection) tempContainer.appendChild(allProductsSection);
    } else {
...
            try {
                // ... (switch statement remains exactly the same as before) ...
                 switch (section.type) {
                 case 'promo_slider':
                     if (section.groupId) {
                         // Pass layoutId for interval management
...
    // --- Update Real DOM and Cache ---
    const generatedHTML = tempContainer.innerHTML;
    homeSectionsContainer.innerHTML = generatedHTML; // Put generated content into the actual container
...
    } catch (error) {
        console.error("Error saving home content to localStorage:", error);
        // Clear cache if saving failed (e.g., storage full)
...
// Helper function to re-initialize sliders after loading from cache
// This is a basic example and might need adjustments
function reinitializeSlidersFromCache(container) {
    Object.values(state.sliderIntervals || {}).forEach(clearInterval); // Clear existing intervals
    state.sliderIntervals = {}; // Reset intervals object

    container.querySelectorAll('[id^="promoSliderLayout_"]').forEach(sliderContainer => {
        const layoutId = sliderContainer.id.split('_')[1];
        const promoCardElement = sliderContainer.querySelector('.promo-card-grid-item');
...
        // Fetch necessary data (card count) - THIS IS THE CHALLENGE with HTML cache
        // A better approach would store card data with the cache or re-fetch minimal data
        // For now, we'll assume the number of cards needs to be known or is small
        const cardCount = promoCardElement?.dataset.cardCount || 1; // Need a way to store/retrieve this

        if (promoCardElement && cardCount > 1 && prevBtn && nextBtn) {
            let currentIndex = 0; // Assume starts at 0
            const imgElement = promoCardElement.querySelector('.product-image');

            const updateImageFromCache = (index, cardsMeta) => {
                // This part is tricky without the original card data.
                // We might need to embed image URLs or IDs in the cached HTML
                // Or, more robustly, store JSON data in cache instead of HTML.
                // For simplicity, this example won't actually change the image on re-init.
                console.log(`Slider ${layoutId}: Should update image to index ${index}`);
            };

            const rotate = () => {
                if (!document.getElementById(sliderContainer.id) || !state.sliderIntervals || !state.sliderIntervals[layoutId]) {
...
        // Re-attach card click listener (needs card data or embedded attributes)
        // Simplified example: Assumes target category ID is stored in data attribute
        if (promoCardElement) {
            promoCardElement.onclick = async (e) => {
                if (!e.target.closest('button')) {
                    // Need a way to get the target category ID for the current slide
                    // Example: Assume it's stored on the img element
                    const imgElement = promoCardElement.querySelector('.product-image');
                    const targetCategoryId = imgElement?.dataset.categoryId; // Requires adding data-category-id="..." during creation

                    if (targetCategoryId) {
                        await navigateToFilterCore({ category: targetCategoryId, subcategory: 'all', subSubcategory: 'all', search: '' });
                        await updateProductViewUI(true);
                    } else {
                        console.warn("Cannot navigate from cached promo card: target category ID not found.");
                    }
                }
            };
        }
...
    // Store card data locally for this slider instance
...
    // Store card count for potential cache re-initialization
    promoCardElement.dataset.cardCount = cards.length;
...
        // Store target category ID on the image for cache re-initialization click handling
        imgElement.dataset.categoryId = currentCard.categoryId || '';
    };
...
            const targetCategoryId = currentCard?.categoryId; // Use optional chaining
...
                console.warn(`Target category ${targetCategoryId} for promo card not found or invalid.`);
            }
        }
    });
...
     const rowTitle = (sectionNameObj && sectionNameObj[state.currentLanguage]) || (rowData.title && rowData.title[state.currentLanguage]) || (rowData.title && rowData.title.ku_sorani) || 'ڕیزی کارت';
