...
        btn.onclick = async () => {
             // *** GUHERTIN DESTPÊ DIKE: Kontrol bike heke jixwe çalak be ***
             if (state.currentCategory === cat.id) {
                 // Jixwe li ser vê kategoriyê ye, tiştek neke
                 return; 
             }
             // *** GUHERTIN DAWÎ DIBE ***
...
    
    // *** GUHERTIN DESTPÊ DIKE: Kontrolkirina valahiyê berî her tiştî ***
    const isHomeContainerEmpty = homeSectionsContainer.innerHTML.trim() === '' || homeSectionsContainer.querySelector('#loader'); // Kontrol bike ka vala ye *berî* ku tu tiştekî bikî
    // *** GUHERTIN DAWÎ DIBE ***
...
        // *** GUHERTIN DESTPÊ DIKE: Tenê loader nîşan bide heke vala be ***
        // For home view, show a loader *inside* the home container
        if (isHomeContainerEmpty) { // Tenê loader nîşan bide heke konteynir bi rastî vala be
...
        
        // *** GUHERTIN DESTPÊ DIKE: Tenê naveroka malê bar bike heke vala be ***
        // Render home content ONLY if new search OR container is empty/has loader
        if (isNewSearch || isHomeContainerEmpty) { 
            if (isHomeContainerEmpty) { // Tenê render bike heke bi rastî vala bû
                await renderHomePageContentUI(); // Render home content (defined below)
            } else {
                console.log("Beşên malê jixwe hatine barkirin. Barkirina dubare hate sekinandin.");
                // Ger loader hebû (ku naha nabe), jê bibe
                const loader = homeSectionsContainer.querySelector('#loader');
