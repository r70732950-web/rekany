...
// Fonksiyon û mentiqê serekî yê bernameyê (Çakkirî bo çareserkirina کێشەی گەڕانەوەی سکڕۆڵ)
...
// *** GORRANKARIYA 1: Fenkşena applyFilterState nûve bike ***
async function applyFilterState(filterState, fromPopState = false) {
    ...
    renderMainCategories(); // Nûvekirina bişkoka kategoriya serekî ya çalak
    await renderSubcategories(state.currentCategory); // Nîşandana barê jêr-kategoriyan (dibe ku vala be)

    // 3. Anîn û nîşandana berheman (fromPopState bişîne)
    // *** Guhertin li vir: scrollYToRestore bişîne ***
    await searchProductsInFirestore(state.currentSearch, true, fromPopState, filterState.scroll);

    // 4. Skrolkirin (lêpirsîn bar kir bo searchProductsInFirestore)
    // JI VIR HAT RAKIRIN
}
...
function renderProducts() {
    // If it's a new render (not appending), clear the container first
    // Note: searchProductsInFirestore already handles clearing for isNewSearch
    // productsContainer.innerHTML = ''; // This line might be redundant now

    if (!state.products || state.products.length === 0) {
        // If there are no products (e.g., after clearing), ensure container is empty
        productsContainer.innerHTML = '';
        return;
    }

    // Only append products that are not already in the DOM
...
// *** GORRANKARIYA 3: Fenkşena searchProductsInFirestore nûve bike ***
// *** Parametran lê zêde bike: fromPopState = false, scrollYToRestore = 0 ***
async function searchProductsInFirestore(searchTerm = '', isNewSearch = false, fromPopState = false, scrollYToRestore = 0) {
    const homeSectionsContainer = document.getElementById('homePageSectionsContainer');
    const scrollTrigger = document.getElementById('scroll-loader-trigger');
    const shouldShowHomeSections = !searchTerm && state.currentCategory === 'all' && state.currentSubcategory === 'all' && state.currentSubSubcategory === 'all';

    // Beşa destpêkê ya ji bo nîşandana beşên rûpela serekiyê wek xwe dimîne
    ...
    // Beşa kontrolkirina Cache wek xwe dimîne, lê skrolkirinê lê zêde bike
    if (isNewSearch && state.productCache[cacheKey]) {
        ...
        renderProducts(); // Nîşandana berhemên ji cache
        scrollTrigger.style.display = state.allProductsLoaded ? 'none' : 'block';

        // *** Skrolkirinê li vir zêde bike piştî barkirina ji cache ***
        if (fromPopState && typeof scrollYToRestore === 'number') {
             // Dibe ku pêwîst be demek dirêjtir bête danîn piştî barkirina ji cache
            setTimeout(() => window.scrollTo(0, scrollYToRestore), 100);
        } else if (!fromPopState) {
            // Skrol bike bo jor tenê eger filterek nû be, ne ji popstate
            // *** Guhertin: Kontrol bike ku isNewSearch jî rast be ***
            if (isNewSearch) {
                 window.scrollTo({ top: 0, behavior: 'smooth' });
            }
        }
        return; // Ji fonksyonê derkeve piştî barkirina ji cache
    }
    ...
    if (isNewSearch) {
        ...
        state.products = [];
        // Clear productsContainer explicitly for new search before showing skeleton
        productsContainer.innerHTML = '';
    ...
    loader.style.display = 'block'; // Nîşandana loaderê ji bo barkirina zêdetir an lêgerîna nû
    ...
    try {
        ...
        if (isNewSearch) {
            state.products = newProducts;
             // Ensure container is clear before rendering new search results
            productsContainer.innerHTML = '';
        } else {
            // Tenê berhemên nû lê zêde bike ku berê tune ne (ji bo pêşîgirtina dubarebûnê)
        ...
        // Beşa nûvekirina cache wek xwe dimîne
        if (isNewSearch) {
        ...
        // *** Berheman nîşan bide PÊŞÎYA vegerandina skrolê ***
        // Ensure skeleton is hidden before rendering actual products
        skeletonLoader.style.display = 'none';
        productsContainer.style.display = 'grid'; // Ensure grid display is set
        renderProducts(); // Renders only NEW products if not isNewSearch

        // Beşa nîşandana peyama "هیچ کاڵایەک نەدۆزرایەوە" wek xwe dimîne
        ...
        // *** Mentiqê skrolkirinê li vir, PIŞTÎ nîşandanê ***
        if (fromPopState && typeof scrollYToRestore === 'number') {
            // Demek kurt bide da ku piştrast bî nîşandan temam bûye
            setTimeout(() => window.scrollTo(0, scrollYToRestore), 100); // Increased timeout slightly
        } else if (isNewSearch && !fromPopState) {
            // Skrol bike bo jor tenê ji bo lêgerîn/filterên nû yên rastî, ne dema vedigerî
            // *** Tenê skrol bike heger ne ji popstate be ***
             window.scrollTo({ top: 0, behavior: 'smooth' });
        }
        ...
        loader.style.display = 'none';
        skeletonLoader.style.display = 'none';
        productsContainer.style.display = 'grid'; // Piştrast be ku konteynir dîsa grid e
    }
}
