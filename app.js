...
        
        let categoryName;
        if (cat.id === 'all') {
            categoryName = t('all_categories_label');
        } else {
            categoryName = cat['name_' + currentLanguage] || cat.name_ku_sorani;
        }
...
function renderMainCategories() {
    ...
    categories.forEach(cat => {
        ...
        // --- START: Fix for 'undefined' category name ---
        let categoryName;
        // If the category is 'all', use the translation key directly
        if (cat.id === 'all') {
            categoryName = t('all_categories_label');
        } else {
            // For other categories from Firestore, get the name as before
            categoryName = cat['name_' + currentLanguage] || cat.name_ku_sorani;
        }
        // --- END: Fix ---
