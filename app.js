    ...
    try {
        ...
        for (const categoryDoc of categoriesSnapshot.docs) {
            ...
            header.innerHTML = `
                ...
                <a href="#" class="view-all-link" data-category-id="${category.id}">${t('view_all')} <i class="fas fa-chevron-left"></i></a>
            `;
...
async function renderAllProductsSection() {
    ...
    const header = document.createElement('div');
    header.className = 'section-header-main';
    header.innerHTML = `<h3 class="section-title-main">${t('all_products_section_title')}</h3>`;
    sectionContainer.appendChild(header);
    ...
    try {
        const productsQuery = query(
            productsCollection,
            orderBy('createdAt', 'desc'),
            limit(12) // Fetch 12 products for this section
        );
        const productsSnapshot = await getDocs(productsQuery);
        ...
        console.error("Error fetching products for 'All Products' section:", error);
    }
...
        isRenderingHomePage = false;
        setupScrollAnimations(); // Call this after adding new elements to the DOM
    }
}
