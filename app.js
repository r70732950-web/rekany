...
let adminScriptLoaded = false; // -> بۆ زانینا کا فایلا ئەدمینی هاتیە بارکرن

let currentCategory = 'all';
let currentSubcategory = 'all';
let currentSubSubcategory = 'all';

// --- هەلبژارتنا ელემენტێن DOM ---
...
// ===================================================================
// فەنکشنێن گشتی و یێن بەکارهێنەرێ ئاسایی
// ===================================================================
...
function createProductCardElement(product) {
    ...
    productCard.addEventListener('click', (event) => {
        ...
        if (addToCartButton) {
            ...
            // Fêrkirina firoşgeha paceyê (window) ji bo banga fonksiyona editProduct
            if(window.editProduct) window.editProduct(product.id);
        } else if (target.closest('.delete-btn')) {
             // Fêrkirina firoşgeha paceyê (window) ji bo banga fonksiyona deleteProduct
            if(window.deleteProduct) window.deleteProduct(product.id);
...
function updateAdminUI(isAdmin) {
    ...
    // Find all admin management sections in the settings page
    const adminSections = [
        'adminPoliciesManagement', 'adminCategoryManagement', 
        'adminContactMethodsManagement', 'adminSocialMediaManagement', 
...
onAuthStateChanged(auth, async (user) => {
    ...
    if (user && user.uid === adminUID) {
        ...
        // **گرنگ:** لێرەدا فایلا admin.js بار دکەین
        if (!adminScriptLoaded) {
            console.log("Admin logged in. Loading admin-specific script...");
            
            // هنارتنا فەنکشن و گۆڕاوێن پێدڤی بۆ فایلا admin.js
            window.db = db;
            window.auth = auth;
            ...
            script.type = 'module';
            script.onload = () => {
                console.log("admin.js loaded and executed.");
                adminScriptLoaded = true;
                // بانگکرنا فەنکشنی initAdmin پشتی فایل باربوو
                if(window.initAdmin) window.initAdmin();
            };
            script.onerror = () => console.error("Failed to load admin.js");
...
function initializeAppLogic() {
    ...
    onSnapshot(categoriesQuery, (snapshot) => {
        ...
        // This is now shared with admin.js via the window object
        window.categories = categories; 
