...
    // FIX 2: Save scroll position BEFORE opening any popup
    if (!mainPage.classList.contains('page-hidden')) {
    	mainPageScrollPosition = window.scrollY;
    }
    
...
        btn.onclick = () => {
            // FIX 1: Prevent re-fetch if category is already selected
            if (currentCategory === cat.id) {
            	history.back(); // Just close the sheet
            	return;
            }
...
        allBtn.onclick = () => {
        	// FIX 1: Prevent re-fetch if category is already selected
        	if (currentSubSubcategory === 'all') return;
        	
            currentSubSubcategory = 'all';
...
            btn.onclick = () => {
            	// FIX 1: Prevent re-fetch if category is already selected
            	if (currentSubSubcategory === subSubcat.id) return;
...
        allBtn.onclick = () => {
        	// FIX 1: Prevent re-fetch if category is already selected
        	if (currentSubcategory === 'all') return;
        	
            currentSubcategory = 'all';
            currentSubSubcategory = 'all';
...
            subcatBtn.onclick = () => {
            	// FIX 1: Prevent re-fetch if category is already selected
            	if (currentSubcategory === subcat.id) return;
            	
