...
async function searchProductsInFirestore(searchTerm = '', isNewSearch = false) {
    ...
    try {
        // Step 1: Fetch promo cards
        let promoCards = [];
        
        // Fetch promo cards only on a new search and when on the main "all" category
        if (isNewSearch && currentCategory === 'all') {
            ...
            promoCards = promoSnapshot.docs.map(doc => ({
                id: doc.id,
                ...doc.data(),
                isPromoCard: true // Flag to identify promo cards
            }));
        }

        // Step 2: Fetch products with pagination and filtering
        ...
        // Step 3: Combine products and promo cards
        ...
        if (isNewSearch) {
            ...
            products = [...products, ...newProducts]; // Only add new products on subsequent loads
        }
