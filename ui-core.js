...
    // Scroll to top ONLY for NEW page views, not for popstate restoration
    // This check prevents scrolling to top when history.back() is used.
...
        } else { // Filter state for main page (or default home state)
            showPage('mainPage'); // Ensure main page visible (from this file)
            applyFilterStateCore(popState); // Apply state logic (category, search etc.)
            // === KODA ÇAKKIRÎ: Têketina pozîsyona scrollê ===
            // true: Ji ber ku divê dîmen li gorî fîltera hîstorîyê were nûvekirin (nîşandana malperê an na).
            // popState.scroll: Pozîsyona scrollê ya tomarkirî tê derbas kirin.
            await updateProductViewUI(true, popState.scroll); 
        }
