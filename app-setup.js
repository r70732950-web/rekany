...
    productCache: {}, // Ev kaş ji bo lêgerînên nû yên di rûniştina heyî de tê bikar anîn
    currentCategory: 'all',
...
// *** KODA NÛ: Pêşgira kaşê ji bo 5 xulekan ***
export const CACHE_PREFIX = 'product_cache_';
// *** DAWÎYA KODA NÛ ***
...
    },
    
    // *** KODA NÛ: Fonksiyona paqijkirina kaşê ya nûvekirî ***
    clearProductCache: () => {
        console.log("Product cache and home page cleared due to admin action.");
        
        // 1. Paqijkirina kaşê bîra (memory cache)
        state.productCache = {}; 
        
        // 2. Paqijkirina kaşê localStorage (5 xulekî)
        try {
...
                }
            }
            console.log("Cleared all 5-minute product caches from localStorage.");
        } catch (error) {
            console.error("Error clearing product caches from localStorage:", error);
        }
