...
// Pênasekirin û sazkarîyên destpêkê (Nûvekirî bo refactoring)
...
// === START: Import functions needed for globalAdminTools ===
import {
    searchProductsInFirestore,
    clearProductCache,
    setEditingProductId,
    getEditingProductId,
    getCategoriesFromState as getCategories, // Renamed to avoid conflict
    getCurrentLanguage
} from './app-data.js'; // Assuming functions are exported from app-data.js

import {
    showNotification,
    openPopup,
    closeCurrentPopup
} from './app-ui.js'; // Assuming functions are exported from app-ui.js
// === END: Import functions needed for globalAdminTools ===


// Firebase Configuration
const firebaseConfig = {
    apiKey: "AIzaSyBxyy9e0FIsavLpWCFRMqgIbUU2IJV8rqE", // Sensitive data - consider environment variables in production
...
export const shortcutRowsCollection = collection(db, "shortcut_rows"); // Added
export const homeLayoutCollection = collection(db, "home_layout"); // Added
export const settingsCollection = collection(db, "settings"); // Added for convenience
...
// Global State Variables (Mutable) - Keep in setup for easy sharing
export let state = {
    ...
    products: [],
    // allPromoCards: [], // Removed, handled dynamically
    // currentPromoCardIndex: 0, // Removed, handled per slider instance
    // promoRotationInterval: null, // Removed, handled per slider instance
    sliderIntervals: {}, // NEW: To manage multiple slider intervals
    categories: [],
    contactInfo: {}, // Potentially populated from Firestore later
    subcategories: [], // Populated dynamically based on selected main category
...
// DOM Elements Exports (Needed by multiple modules)
...
export const subSubcategoriesContainer = document.getElementById('subSubcategoriesContainer'); // Main page sub-sub container
...
// This ensures admin.js still works without modification
window.globalAdminTools = {
    ...
    // Collections
    productsCollection, categoriesCollection, announcementsCollection, promoGroupsCollection, brandGroupsCollection, shortcutRowsCollection, homeLayoutCollection, settingsCollection,
