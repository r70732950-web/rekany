...
function showPage(pageId) {
    ...
    // زیادکردنی ستەیتی نوێ بۆ history کاتێک پەیج دەگۆڕدرێت
...
// زیادکرا بۆ چارەسەری کێشەی گەڕانەوە
...
// نوێکرایەوە بۆ چارەسەری کێشەی گەڕانەوە
function closeAllPopups() {
    // ئەگەر ستەیتی ئێستا هی پۆپئەپێک بوو، ئەوا بگەڕێوە دواوە
    if (history.state && (history.state.sheet || history.state.modal)) {
        history.back();
    } else {
        // ئەگەرنا، تەنها هەمووی دابخە بێ ئەوەی دەستکاری history بکات
...
function toggleSheet(sheetId, show) {
    ...
    // پارچەیەک زیاد کراوە بۆ ئەوەی لە کاتی کردنەوەی پۆپئەپێک، ئەویتر دابخرێت
    if (show) {
        _closeAllPopupsWithoutHistory();
    }
    ...
    if (show) {
        ...
        // زیادکردنی ستەیتی نوێ بۆ history
        history.pushState({ sheet: sheetId }, '', `#${sheetId}`);

    } else {
        // کاتێک بەکارهێنەر خۆی دایدەخات، ئێمە بانگی history.back() دەکەین
        // ئەمەش وا دەکات popstate event چالاک بێت و پۆپئەپەکە دابخرێت
        closeAllPopups();
    }
}
...
// =======================================================
// کۆدی چارەسەری کێشەی دوگمەی گەڕانەوە (Back Button)
// =======================================================

// لە یەکەمجاردا ستەیتی سەرەتایی پەیجەکە دادەنێین
history.replaceState({ page: 'mainPage' }, 'Home', '#mainPage');

window.addEventListener('popstate', (event) => {
    // کاتێک بەکارهێنەر دوگمەی گەڕانەوە دادەگرێت، ئەم فەنکشنە کار دەکات
    const state = event.state;
    
    // سەرەتا هەموو پۆپئەپەکان دادەخەین
    _closeAllPopupsWithoutHistory();
    
    if (state) {
        if (state.page) {
            // ئەگەر ستەیتەکە هی پەیجێک بوو، ئەو پەیجە پیشان بدە
            document.querySelectorAll('.page').forEach(page => {
                page.classList.toggle('page-hidden', page.id !== state.page);
            });
            updateActiveNav(state.page === 'settingsPage' ? 'settingsBtn' : 'homeBtn');
        } else if (state.sheet) {
            // ئەگەر ستەیتەکە هی sheet بوو، دووبارە بیکەرەوە
            // ئەمە بۆ ئەوەیە ئەگەر بەکارهێنەر دوگمەی forward ی بەکارهێنا
            toggleSheet(state.sheet, true);
        }
    } else {
        // ئەگەر هیچ ستەیتێک نەبوو، بگەڕێوە بۆ پەیجی سەرەکی
        showPage('mainPage');
        updateActiveNav('homeBtn');
    }
});