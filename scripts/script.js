...
// ==========================================================
// ===== START OF THE ONLY MAJOR CHANGE - NEW AUTH LOGIC ====
// ==========================================================
onAuthStateChanged(auth, async (user) => {
    // UIDـی تایبەتی ئەدمین لێرە دیاری کراوە
    const adminUID = "xNjDmjYkTxOjEKURGP879wvgpcG3"; 
  
    if (user) {
      // Em piştrast dikin ka UIDـa bikarhênerê têketî heman UIDـa admin e yan na
      // Ev rêbaza herî parastî ye bo zanîna adminbûnê
      ...
      // Eger bikarhêner ne têketî be, ew ne admin e
      isAdmin = false;
      sessionStorage.removeItem('isAdmin');
    }
  
    // Piştî piştrastkirinê, em UI nû dikin
    updateAdminUI(isAdmin);
    // renderProducts() is called inside updateAdminUI implicitly if needed or should be called after
    // Let's call it explicitly to be sure the view updates on login/logout
    renderProducts(); 
});
// ==========================================================
// ===== END OF THE ONLY MAJOR CHANGE - NEW AUTH LOGIC ====
// ==========================================================
...
        renderProducts();
        // updateAdminUI is already called by onAuthStateChanged, but we can check session storage for initial load
...
    // The onAuthStateChanged function is now defined above and will handle auth state globally.
    // We don't need the old logic here anymore.
