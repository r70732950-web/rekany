...
// Em fonksiyonên nû yên Storage û Chat ji 'globalAdminTools' import dikin
// ئێمە فەنکشنە نوێیەکانی ستۆرێج و چات لە 'globalAdminTools' هاوردە دەکەین
const {
    ...
    // Fonksiyon û Koleksiyonên Chatê yên Nû (فەنکشن و کۆڵێکشنە نوێیەکانی چات)
    chatsCollection, usersCollection, 
    listenForAdminChatList, listenForChatMessages, sendChatMessageCore, markChatAsReadCore,
    // Fonksiyonên Core yên Heyî (فەنکشنە Coreـەکانی پێشوو)
    setEditingProductId, getEditingProductId, getCategories, getCurrentLanguage,
    clearProductCache
} = window.globalAdminTools;
// === END: KODA NÛ / کۆتایی کۆدی نوێ ===

// Global variable ji bo unsubscribekirina chatê ya admin
// گڵۆباڵ ڤاریابڵ بۆ unsibscribeـی چاتی ئەدمین
...
window.AdminLogic = {
    ...
    initialize: function() {
        ...
        // === START: KODA NÛ / کۆدی نوێ ===
        this.renderAdminChatListUI(); // Lîsteya chatê ya admin bar bike (لیستی چاتی ئەدمین بار بکە)
        // === END: KODA NÛ / کۆتایی کۆدی نوێ ===
    },
    ...
    deinitialize: function() {
        ...
        // === START: KODA NÛ / کۆدی نوێ ===
        // Dema ku admin derdikeve, guhdarê chatê rake (کاتێک ئەدمین دەردەچێت، گوێگری چات لاببە)
        if (activeAdminChatListener) {
            activeAdminChatListener();
            activeAdminChatListener = null;
        }
        // === END: KODA NÛ / کۆتایی کۆدی نوێ ===
    },
    ...
    // === START: KODÊN NÛ JI BO CHAT ADMIN / کۆدی نوێ بۆ چاتی ئەدمین ===
    
    /**
     * Lîsteya chatên bikarhêneran ji bo admin bar dike
     * لیستی چاتی بەکارهێنەران بۆ ئەدمین بار دەکات
     */
    renderAdminChatListUI: function() {
        const container = document.getElementById('adminChatListContainer');
        container.innerHTML = '<p>...خەریکی بارکردنی لیستی نامەکانە</p>';

        listenForAdminChatList((chatList, error) => {
            if (error) {
                container.innerHTML = '<p style="color: red;">هەڵە لە بارکردنی نامەکان.</p>';
                return;
            }
            if (chatList.length === 0) {
                container.innerHTML = '<p>هیچ نامەیەک نییە.</p>';
                return;
            }
            ...
            chatList.forEach(chat => {
                const item = document.createElement('div');
                item.className = 'admin-chat-list-item';
                item.dataset.chatId = chat.id;
                item.dataset.userName = chat.userName || chat.userPhone || 'بەکارهێنەری نەناسراو';
                
                const time = chat.lastTimestamp ? new Date(chat.lastTimestamp.toDate()).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }) : '';
                const unread = !chat.seenByAdmin;

                item.innerHTML = `
                    <div class="chat-list-info">
                        <div class="chat-list-user">${chat.userName || chat.userPhone}</div>
                        <div class="chat-list-preview ${unread ? 'unread' : ''}">
                            ${chat.lastMessage || '...'}
                        </div>
                    </div>
                    <div class="chat-list-meta">
                        <span class="chat-list-time">${time}</span>
                        ${unread ? '<span class="chat-list-badge">!</span>' : ''}
                    </div>
                `;
                container.appendChild(item);
            });
    ...
    /**
     * Dema ku admin li ser chatekê bitikîne, UIya chatê vedike
     * کاتێک ئەدمین کلیک لە چاتێک دەکات، UIـی چاتەکە دەکاتەوە
     */
    openAdminChatUI: function(chatRoomId, userName) {
        // Guhdarê kevn rake (گوێگری کۆن لاببە)
        if (activeAdminChatListener) {
            activeAdminChatListener();
            activeAdminChatListener = null;
        }
        ...
        const chatTitle = chatSheet.querySelector('.sheet-title span');
        const chatMessagesContainer = document.getElementById('chatMessagesContainer');
        const chatMessageForm = document.getElementById('chatMessageForm');
        const chatMessageInput = document.getElementById('chatMessageInput');
        const chatSendBtn = document.getElementById('chatSendBtn');

        // Sernavê nû bike (ناونیشان نوێ بکەوە)
        chatTitle.textContent = `گفتوگۆ لەگەڵ ${userName}`;
        chatMessagesContainer.innerHTML = `<div id="chatLoadingSpinner" style="text-align: center; padding: 30px;"><i class="fas fa-spinner fa-spin fa-2x" style="color: var(--dark-gray);"></i></div>`;
        
        // Guhdarê nû saz bike (گوێگری نوێ دابنێ)
        activeAdminChatListener = listenForChatMessages(chatRoomId, (messages, error) => {
            document.getElementById('chatLoadingSpinner').style.display = 'none';
            if (error) {
                chatMessagesContainer.innerHTML = `<p style="text-align:center;">${t('error_generic')}</p>`;
                return;
            }
            this.renderAdminMessagesUI(messages); // Peyaman nîşan bide (نامەکان پیشان بدە)
            
            // Chatê wekî "xwendî" nîşan bide (چاتەکە وەک "خوێندراوە" نیشان بدە)
            markChatAsReadCore(chatRoomId, 'admin');
        });

        // Forma şandinê ji bo admin saz bike (فۆڕمی ناردن بۆ ئەدمین دابنێ)
        // Pêşî guhdarê kevn rake (سەرەتا گوێگری کۆن لاببە)
        const newForm = chatMessageForm.cloneNode(true);
        chatMessageForm.parentNode.replaceChild(newForm, chatMessageForm);
        
        newForm.onsubmit = async (e) => {
            e.preventDefault();
            const newChatInput = document.getElementById('chatMessageInput'); // Inputa nû bistîne (Inputـە نوێیەکە وەربگرە)
            const newSendBtn = newForm.querySelector('button[type="submit"]'); // Bişkoka nû bistîne (دوگمە نوێیەکە وەربگرە)
            ...
            try {
                // Bişîne wekî 'admin' (وەک 'admin' بینێرە)
                await sendChatMessageCore(tempMessage, chatRoomId, 'admin'); 
            } catch (error) {
                showNotification(t('error_generic'), 'error');
                newChatInput.value = tempMessage; 
            } finally {
    ...
    /**
     * Peyamên chatê di UIya admin de nîşan dide
     * نامەکانی چات لە UIـی ئەدمین پیشان دەدات
     */
    renderAdminMessagesUI: function(messages) {
        const chatMessagesContainer = document.getElementById('chatMessagesContainer');
        chatMessagesContainer.innerHTML = ''; 
        if (messages.length === 0) {
            chatMessagesContainer.innerHTML = `<div class="cart-empty"><i class="fas fa-comments"></i><p>هیچ نامەیەک نییە.</p></div>`;
        }

        messages.forEach(msg => {
            const msgElement = document.createElement('div');
            
            // Admin her gav 'user' e, yên din 'admin' in (ji perspektîfa admin)
            // ئەدمین هەمیشە 'user'ـە، ئەوانی تر 'admin'ـن (لە ڕوانگەی ئەدمینەوە)
            const senderType = (msg.senderId === 'admin') ? 'user' : 'admin';
            
            msgElement.className = `chat-message ${senderType}`;
            
            const bubble = document.createElement('div');
            bubble.className = 'chat-bubble';
            bubble.innerHTML = formatDescription(msg.text); 
    ...
    // === END: KODÊN NÛ JI BO CHAT ADMIN / کۆتایی کۆدی نوێ بۆ چاتی ئەدمین ===
