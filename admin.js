...
export function initAdmin(db, auth, showNotification, t, closeCurrentPopup, openPopup, categories, createProductImageInputs, populateSubcategoriesDropdown, populateSubSubcategoriesDropdown) {
    ...
    // =======================================================
    // Event Listenerـێن تایبەت ب ئەدمینی
    // =======================================================
    ...
    window.deleteProduct = async (productId) => {
        ...
        } catch (error) { showNotification(t('product_delete_error'), 'error'); }
    };
        ...
        for (const mainDoc of mainCategoriesSnapshot.docs) {
            ...
            for (const subDoc of subCategoriesSnapshot.docs) {
                ...
                for (const subSubDoc of subSubCategoriesSnapshot.docs) {
                    ...
                    content += `<div class="category-manage-item" style="margin-right: 40px; padding: 8px; border-right: 2px solid #e2e8f0; margin-bottom: 8px;"><div style="display: flex; justify-content: space-between; align-items: center;"><span>-- ${subSubCategory.name_ku_sorani} (Rêz: ${subSubCategory.order || 0})</span><div><button class="edit-btn small-btn" data-path="${subSubPath}" data-level="3"><i class="fas fa-edit"></i></button><button class="delete-btn small-btn" data-path="${subSubPath}" data-name="${subSubCategory.name_ku_sorani}"><i class="fas fa-trash"></i></button></div></div></div>`;
                }
            }
        }
    ...
    async function openEditCategoryModal(docPath, level) {
        ...
        iconField.style.display = level === '1' ? 'block' : 'none';
        if (level === '1') document.getElementById('editCategoryIcon').value = category.icon || '';
        
        openPopup('editCategoryModal', 'modal');
    }
    
    async function handleDeleteCategory(docPath, categoryName) {
        if (confirm(`تو پشتڕاستی دێ جورێ "${categoryName}" ژێبەى؟\nئاگەهداربە: ئەڤ کارە دێ هەمی جورێن لاوەکی ژی ژێبەت.`)) {
            try {
    ...
    function renderAdminAnnouncementsList() {
        ...
        onSnapshot(q, (snapshot) => {
            ...
            snapshot.forEach(doc => {
                ...
                item.innerHTML = `<div class="admin-notification-details"><div class="notification-title">${announcement.title.ku_sorani}</div></div><button class="delete-btn"><i class="fas fa-trash"></i></button>`;
    ...
    function renderSocialMediaLinksAdmin() {
        ...
        onSnapshot(q, (snapshot) => {
            ...
            snapshot.forEach(doc => {
                ...
                item.innerHTML = `<div class="item-info"><i class="${link.icon}"></i><div class="item-details"><span class="item-name">${link.name_ku_sorani}</span><span class="item-value">${link.url}</span></div></div><button class="delete-btn"><i class="fas fa-trash"></i></button>`;
    ...
    function renderContactMethodsAdmin() {
        ...
        onSnapshot(q, (snapshot) => {
            ...
            snapshot.forEach(doc => {
                ...
                const item = document.createElement('div');
                item.className = 'social-link-item';
                item.innerHTML = `<div class="item-info"><i class="${method.icon}" style="color: ${method.color};"></i><div class="item-details"><span class="item-name">${method.name_ku_sorani}</span><span class="item-value">${method.value}</span></div></div><button class="delete-btn"><i class="fas fa-trash"></i></button>`;
    ...
    function populateAdminCategoryDropdowns() {
        const selects = [document.getElementById('parentCategorySelect'), document.getElementById('parentMainCategorySelectForSubSub'), document.getElementById('promoCardTargetCategory')];
