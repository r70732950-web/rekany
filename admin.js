...
export function initAdmin(db, auth, showNotification, t, closeCurrentPopup, openPopup, categories) {
    ...
    // پانێلێن ئەدمینی
    const adminPanels = [
        'adminCategoryManagement', 'adminContactMethodsManagement', 
        'adminSocialMediaManagement', 'adminAnnouncementManagement', 
        'adminPoliciesManagement', 'adminPromoCardsManagement'
    ];
    ...
    let unsubscribers = []; // لیستا هەمی onSnapshot listeners
    ...
    async function handleProductFormSubmit(e) {
        ...
        const productData = {
            name: { ku_sorani: document.getElementById('productNameKuSorani').value, ku_badini: document.getElementById('productNameKuBadini').value, ar: document.getElementById('productNameAr').value },
        ...
        try {
            ...
            closeCurrentPopup();
        } catch (error) { showNotification(t('error_generic'), 'error'); console.error("Error saving product:", error);
        } finally { submitButton.disabled = false; submitButton.textContent = editingProductId ? 'نویکرنەوە' : 'پاشەکەفتکرن'; editingProductId = null; }
    }
    ...
    // =======================================================
    window.editProduct = async (productId) => { const productRef = doc(db, "products", productId); const productSnap = await getDoc(productRef); if (!productSnap.exists()) return showNotification(t('product_not_found_error'), 'error'); const product = { id: productSnap.id, ...productSnap.data() }; editingProductId = productId; formTitle.textContent = 'دەستکاریکرنا کاڵای'; productForm.reset(); document.getElementById('productNameKuSorani').value = product.name?.ku_sorani || ''; document.getElementById('productNameKuBadini').value = product.name?.ku_badini || ''; document.getElementById('productNameAr').value = product.name?.ar || ''; document.getElementById('productPrice').value = product.price; document.getElementById('productOriginalPrice').value = product.originalPrice || ''; document.getElementById('productCategoryId').value = product.categoryId; document.getElementById('productDescriptionKuSorani').value = product.description?.ku_sorani || ''; document.getElementById('productDescriptionKuBadini').value = product.description?.ku_badini || ''; document.getElementById('productDescriptionAr').value = product.description?.ar || ''; createProductImageInputs(product.imageUrls || []); document.getElementById('productExternalLink').value = product.externalLink || ''; document.getElementById('shippingInfoKuSorani').value = product.shippingInfo?.ku_sorani || ''; document.getElementById('shippingInfoKuBadini').value = product.shippingInfo?.ku_badini || ''; document.getElementById('shippingInfoAr').value = product.shippingInfo?.ar || ''; await populateSubcategoriesDropdown(product.categoryId, product.subcategoryId); await populateSubSubcategoriesDropdown(product.categoryId, product.subcategoryId, product.subSubcategoryId); openPopup('productFormModal', 'modal'); };
    ...
    function renderAdminAnnouncementsList() { const container = document.getElementById('announcementsListContainer'); const q = query(collection(db, "announcements"), orderBy("createdAt", "desc")); const unsubscribe = onSnapshot(q, (snapshot) => { container.innerHTML = ''; if (snapshot.empty) { container.innerHTML = `<p style="text-align:center;">${t('no_announcements_sent')}</p>`; return; } snapshot.forEach(doc => { const announcement = { id: doc.id, ...doc.data() }; const item = document.createElement('div'); item.className = 'admin-notification-item'; item.innerHTML = `<div class="admin-notification-details"><div class="notification-title">${announcement.title.ku_sorani}</div></div><button class="delete-btn"><i class="fas fa-trash"></i></button>`; item.querySelector('.delete-btn').addEventListener('click', () => deleteAnnouncement(announcement.id)); container.appendChild(item); }); }); unsubscribers.push(unsubscribe); }
    ...
    // فەنکشنا پاککرنێ: دەمێ ئەدمین چوو دەر، هەمی real-time listenerـان ڕادوەستینیت
